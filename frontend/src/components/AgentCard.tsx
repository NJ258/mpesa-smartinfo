...
const AgentCard = ({ agent, onSelect }: { agent: Agent; onSelect: () => void }) => {
  return (
    <button
      type="button"
      onClick={onSelect}
      className="group relative flex w-full flex-col rounded-[1.8rem] border border-slate-100 bg-white p-5 text-left shadow-sm transition-all duration-300 hover:border-mpesaRed/30 hover:shadow-md active:scale-[0.99]"
    >
      ...
      <div className="mt-4 flex items-center justify-between border-t border-slate-50 pt-3 text-xs text-slate-500">
        <span className="font-semibold bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full flex items-center gap-1">
